...
static int
newmappages(pde_t *pgdir, void *va, uint size, uint pa, int perm)
{
  char *a, *last;
  pte_t *pte;
...
int
newallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  char *mem;
  uint a;
  ...
  for(; a < newsz; a += PGSIZE){
    ...
    // }
    // memset(mem, 0, PGSIZE);
    if(newmappages(pgdir, (char*)a, PGSIZE, 0, PTE_W|PTE_U) < 0){
